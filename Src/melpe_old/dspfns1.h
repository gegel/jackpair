...
#else  //ndef __TARGET_FEATURE_DSPMUL
 ...
 * saturation.
 */
__ARM_INTRINSIC int16_t _melpe_saturate(int32_t x)
{
    ...
    if (x > INT16_MAX || x < INT16_MIN)
        x = __qdbl(INT32_MAX - ((x) >> 31)) >> 16;   /* _melpe_saturate and set Overflow */
    return (int16_t) x;
#endif
}
 ...
 * is negative). _melpe_saturate on overflow.
 */
__ARM_INTRINSIC int16_t melpe_shl(int16_t x, int16_t shift)
{
    ...
    if (shift > 15)
        shift = 16;
    return _melpe_saturate(x << shift);
}
 ...
 * 15 and _melpe_saturate it. (Saturation is only necessary if both inputs
 * were -0x8000, in which case the result "should" be 0x8000 and is
 * _melpe_saturated to 0x7FFF.)
 */
...
/*
 * Multiply two 16-bit signed integers, shift the result right by
 * 15 with rounding, and _melpe_saturate it. (Saturation is only necessary
 * if both inputs were -0x8000, in which case the result "should"
 * be 0x8000 and is _melpe_saturated to 0x7FFF.)
 */
...
/*
 * Return a _melpe_saturated value appropriate to the most recent carry-
 ...
 * 
 * After division, the result is _melpe_saturated to fit into a 16-bit
